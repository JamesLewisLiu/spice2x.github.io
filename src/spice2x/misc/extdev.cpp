...
static long __cdecl gfdm_unit_get_input_p(int device, size_t player) {
    ...
    if (games::gitadora::is_drum()) {
        ...
        // Accept both the configured hardware-style Coin binding and the short
        // pulse generated by F1/F9/API coin insertion.
        if (Buttons::getState(RI_MGR, buttons.at(gitadora_button_mapping[2])) ||
                eamuse_coin_get_insert_pulse()) {
            ret |= 0x10;
        }
    ...
    if (games::gitadora::is_guitar()) {
        ...
        // pulse generated by F1/F9/API coin insertion.
        if (device == 0 &&
                (Buttons::getState(RI_MGR, buttons.at(gitadora_button_mapping[2])) ||
                 eamuse_coin_get_insert_pulse())) {
            ret |= 0x10;
        }
